...
with st.container(border=True):
  ...
  with m1:
      st.write(" ")
  with m2:
      st.markdown("""This section is under works and shall be released with
...
# you can use as_json=True to get all codes in json format
...
with st.container(border=True):
  st.header('A. Markets & Exchanges')
  st.info(""" Compare Global Markets. Investigate each Markets performance
    in the tabs which follow""")
  tub0, tub1, tub1A, tub2, tub3, tub4, tub5, tub6, tub7 = st.tabs(
    ['Global Markets', "NSE - IN", "BSE - SENSEX", "SPX - USA", "DAX - GDR",
    'CAC40 - FR', 'Dow Jones - US', 'Nikkei225 - JPN', "FTSE - UK"])
  with tub0:
...
with st.container(border=True):
  st.header("B. SIP Calculator")
  st.caption("Find out your Returns from any SIP scheme against a one-time investment")
  with st.form('sipcalc'):
    ...
    YR = st.slider("Enter the yearly Rate of Return in pct: ",
                   min_value=5,
...
with st.container(border=True):
  st.header('D. Exchange Traded & Index Funds')
...
with st.container(border=True):
  ...
  with cn2:
      st.write(" ")
      st.markdown('''Estimate Uncertainty Levels in the Markets to
...
with st.container(border=True):
  ...
  with bn2:
      st.write(" ")
      st.markdown('''Estimate the real Risk-Free rate >> Yield of the
