...
try:
   ...
   eps = 'Data Unreported'
try:
    cash_and_cash_eqv2 = bsheet2.loc['Cash Cash Equivalents & Short Term Investments'][0]
except Exception:
...
try:
    acr = cash_and_cash_eqv2/current_liabilities
except Exception:
...
with st.container(border=True):
	...
	st.markdown("""A Firm with a CR less than one means that its Equity heavy and
...
with st.container(border=True):
	...
	with g7:
	    st.metric("Cash Ratio)", round(c_ratio2, 4))
...
with st.container(border=True):
	...
	with h14:
	    ...
	    st.write('A High ICR i.e. above 2.00 exhibits a firm\'s capacity to pay off all interests of short term debts')
	with h15:
